...
public partial class ResXResourceReader : IResourceReader
{
    ...
    public string BasePath
    {
        ...
        set
        {
            if (_isReaderDirty)
            {
                throw new InvalidOperationException("Property can be changed only before the enumeration.");
            }
    ...
    private void ParseXml(XmlTextReader reader)
    {
        ...
        try
        {
            ...
            catch (SerializationException se)
            {
                ...
                string newMessage = string.Format("Type {0} could not be read from the data in line {1}, position {2}.  The type's internal structure may have changed.  Either implement ISerializable on the type or provide a type converter that can provide a more reliable conversion format, such as text or an array of bytes.  The conversion exception was: {3}", reader[ResXResourceWriter.TypeStr], pt.Y, pt.X, se.Message);
            ...
            {
                Point pt = GetPosition(reader);
                string newMessage = string.Format("Type {0} in the data at line {1}, position {2}, cannot be loaded because it threw the following exception during construction: {3}", reader[ResXResourceWriter.TypeStr], pt.Y, pt.X, tie.InnerException.Message);
            ...
            {
                throw new ArgumentException(string.Format("ResX file {0} cannot be parsed.", e.Message), e);
            }
            catch (Exception e)
            {
                ...
                else
                {
                    ...
                    throw new ArgumentException(string.Format("ResX file {0} cannot be parsed.", xmlEx.Message), xmlEx);
                }
            }
        }
        finally
        ...
        if (!validFile)
        {
            _resData = null;
            _resMetadata = null;
            throw new ArgumentException("ResX input is not valid. Cannot find valid \"resheader\" tags for the ResX reader and writer type names.");
        }
    }
    ...
    private void ParseDataNode(XmlTextReader reader, bool isMetaData)
    {
        ...
        {
            throw new ArgumentException(string.Format("Cannot find a name for the resource with the value '{0}'.", nodeInfo.ValueData));
        }
