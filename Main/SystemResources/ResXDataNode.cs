...
public sealed class ResXDataNode : ISerializable
{
    ...
    public ResXDataNode(string name, object value, Func<Type, string> typeNameConverter)
    {
        ...
        {
            throw new InvalidOperationException(string.Format("Item named '{0}' of type '{1}' cannot be added to the resource file because it is not serializable.", name, valueType.FullName));
        }
    ...
    private void FillDataNodeInfoFromObject(DataNodeInfo nodeInfo, object value)
    {
        ...
        else
        {
            ...
            if (value is not null && !valueType.IsSerializable)
            {
                throw new InvalidOperationException(string.Format("Item named '{0}' of type '{1}' cannot be added to the resource file because it is not serializable.", _name, valueType.FullName));
            }
...
#pragma warning restore SYSLIB0011 // Type or member is obsolete
            ...
            else if (string.Equals(mimeTypeName, ResXResourceWriter.ByteArraySerializedObjectMimeType))
            {
                if (!string.IsNullOrEmpty(typeName))
                {
                    ...
                    {
                        string newMessage = string.Format("Type {0} in the data at line {1}, position {2} cannot be located.", typeName, dataNodeInfo.ReaderPosition.Y, dataNodeInfo.ReaderPosition.X);
            ...
            if (type is not null)
            {
                ...
                else
                {
                    ...
                    if (tc.CanConvertFrom(typeof(string)))
                    {
                        ...
                        {
                            string newMessage = string.Format("The type {0} on line {1}, position {2} threw the following exception while being converted: {3}", typeName, dataNodeInfo.ReaderPosition.Y, dataNodeInfo.ReaderPosition.X, nse.Message);
    ...
    public object GetValue(ITypeResolutionService typeResolver)
    {
        ...
        if (FileRefFullPath is not null)
        {
            ...
            {
                string newMessage = string.Format("Type {0} cannot be located.", FileRefType);
